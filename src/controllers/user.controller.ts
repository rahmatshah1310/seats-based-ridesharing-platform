import { createUser, getUserById, getAllUsers, getUserByRole, deleteUsers,loginUser,getCurrentUser } from "@/services/user.service";
...
import {serializeUser,serializeDriverProfile,serializePassengerProfile} from "@/helpers/serializeProfile";
...
export const registerUser: Controller = async (req, res) => {
  ...
  try {
    ...
    if (req.files && !Array.isArray(req.files)) {
      ...
      if (body.role === "driver" && req.files.vehicleImages && req.files.vehicleImages.length > 0) {
...
export const login:Controller=async(req,res)=>{
  ...
  try {
   ...
   if (userData.role === "driver" && "driverProfile" in userData &&  userData.driverProfile) {
     serializedProfile = serializeDriverProfile(userData.driverProfile);
   } else if (userData.role === "passenger" && "passengerProfile" in userData &&  userData.passengerProfile) {
